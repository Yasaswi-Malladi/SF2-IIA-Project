...
class Image():
    ...
    def regroup_N(self, N):
        return regroup(self.image, N, False)/N
    ...
    def bits_dct(self, N):
        Ys = np.reshape(self.image, (-1, N, N))
        bits = 0
        for Ynn in Ys:
            bits += bpp(Ynn) * Ynn.size
        return bits
    ...
    def comp_ratio_dct(self, N: int, step :float):
        ...
        def _DCT_optimisation_func(step):
            return abs(reference_rms - self.image.DCT(N, False).quantise(step, False).iDCT(8, False).rms(self.image))
