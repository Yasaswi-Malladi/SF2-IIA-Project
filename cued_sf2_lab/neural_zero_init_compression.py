...
class ZeroInitResidualNet(nn.Module):
    def __init__(self):
        ...
        # --- ZERO-INIT TECHNIQUE ---
        # Initialize the final layer weights and biases to EXACTLY 0.0
        # This guarantees the network starts as an Identity function
...
def run_zero_init_optimization():
    ...
    for epoch in range(PHASE2_EPOCHS):
        ...
        if epoch % 50 == 0 or epoch == PHASE2_EPOCHS - 1:
            ...
            if epoch % 200 == 0 or epoch == PHASE2_EPOCHS - 1:
                print(f"   Epoch {epoch:4d} | Train SSIM: {1-dist_loss.item():.4f} | Total Bits: {total_bits:.0f} | Lambda: {lambda_val:.6f}")
    ...
    print(f"\n[Zero-Init Neural] Total Bits={total_bits:.0f}, SSIM={eval_ssim:.4f}")
    
    fig, axes = plt.subplots(1, 2, figsize=(10, 5))
    fig.suptitle(f"Zero-Init Neural Refinement (Target: 5.0 KB)")
    ...
    axes[1].set_title(f"LBT + Neural Residual\nTotal Bits: {total_bits:.0f} | SSIM: {eval_ssim:.4f}")
