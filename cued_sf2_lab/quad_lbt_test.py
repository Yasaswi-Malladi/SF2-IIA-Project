...
DEPTH_SCALE = 0.85 # QP multiplier per depth level (< 1 = finer for detail)


def block_edge_energy(block):
    """
    Acts as a high-pass filter to detect sharp edges in a block.
    Computes adjacent pixel differences; returns the maximum difference.
    This ignores smooth gradients (which have high variance but low max-gradient).
    """
    if block.shape[0] < 2 or block.shape[1] < 2:
        return 0.0
    gx = np.abs(np.diff(block, axis=1))
    gy = np.abs(np.diff(block, axis=0))
    return max(np.max(gx), np.max(gy))
...
def qt_encode(image, base_qstep, var_thresh):
    ...
    def _encode_leaf(block, size, depth):
        ...
        # 2. Adaptive quantisation — finer for deeper (more detailed) blocks
        q = base_qstep * (DEPTH_SCALE ** depth)
        Yq = quant1(Y, q, q).astype(int)
    ...
    def _split(x, y, size, depth):
        ...
        if size > MIN_BLOCK and depth < MAX_DEPTH and block_edge_energy(block) > var_thresh:
...
def find_qt_params(X, target_bits=TARGET_BITS):
    ...
    H, W = X.shape
    # Edge threshold: 75th percentile of 16x16 sub-block edge energies
    # Higher percentile = less splitting = fewer blocks = less overhead
    block_edges = [block_edge_energy(X[i:i+16, j:j+16])
